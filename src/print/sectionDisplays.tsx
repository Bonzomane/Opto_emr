...
export function PrelimTestsDisplay({ tests }: { tests: PreliminaryTests }) {
  ...
  const stereoDisplay = () => {
    ...
    const parts = [];
    if (tests.stereoFormes) parts.push(`F: ${tests.stereoFormes === 'Normal' ? 'Normal' : tests.stereoFormes + '/8'}`);
    if (tests.stereoCercles) parts.push(`C: ${tests.stereoCercles === 'Normal' ? 'Normal' : tests.stereoCercles + '/10'}`);
    if (tests.stereoAnimaux) parts.push(`A: ${tests.stereoAnimaux === 'Normal' ? 'Normal' : tests.stereoAnimaux + '/3'}`);
    return parts.join(' ');
  };
